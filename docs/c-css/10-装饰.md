...
<img src="./assets/pp9ZeQx-20260817094634407.png" style="zoom:30%;" />
...
<style>
    span {
        ...
        width: 18px;
        height: 24px;
        background-image: url(https://raw.githubusercontent.com/hughxusu/lesson-front-end/refs/heads/develop/static/taobao.png);
...
```mermaid
flowchart LR
   a(登录) --> b(选择图标库) --> c(选择图标加入购物车) --> d(购物车) --> e(添加至项目) --> f(下载至本地)
```
...
<img src="./assets/icon-font.jpg" style="zoom:50%;" />
...
<img src="./assets/piAOHbt.jpg" style="zoom:67%;" />
...
1. 制作一个按钮，当鼠标移入时显示二维码，移除时二维码消失。（二维码可以使用背景图片代替）
1. 制作一个表格，标题为黑底白字，单行为白色，双行为灰色，鼠标移入表格时出现浮点效果。
